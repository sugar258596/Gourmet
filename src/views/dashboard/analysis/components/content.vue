<template>
  <div class="content">
    <div class="title my-3"> {{ title }}</div>
    <div class="grid grid-cols-3 gap-2">
      <template v-for="(v, i) in imageList">
        <div :Key="i" class="bg-purple flex-1 image-box relative cursor-pointer" v-if="i < 3">
          <img class="object-cover w-full h-full min-w-90px no-image" :src="v[url]" alt="" />
          <div
            class="absolute justify-center flex-items-center h-full bottom-0 left-0 right-0 color-slate-100 Prompting"
            >{{ imageList.length - 3 }}</div
          >
        </div>
      </template>
    </div>
  </div>
</template>

<script setup lang="ts">
  import { CardListProps } from '../type';

  withDefaults(defineProps<CardListProps>(), {
    url: 'url',
  });
</script>

<style lang="scss" scoped>
  @import 'style';

  .content {
    display: grid;
    grid-template-rows: 2fr 3fr;
    width: inherit;
  }

  .title {
    font-size: 20px;
    text-indent: 1em;
  }

  .image-box {
    max-width: 93px;
    height: auto;

    &::after {
      content: '';
      position: absolute;
      top: 0;
      left: 0;
      width: 100%;
      height: 100%;
      background: rgb(0 0 0 / 25%); /* 半透明黑色遮罩 */
    }

    &:hover::after {
      background: rgb(0 0 0 / 50%); /* 半透明黑色遮罩 */
    }

    &:nth-child(3n):hover::after {
      background: rgb(0 0 0 / 25%); /* 最后一个子元素不显示遮罩 */
    }

    &:nth-child(3n):hover .Prompting {
      display: flex;
    }

    .Prompting {
      display: none;
      background: rgb(0 0 0 / 50%);
      font-size: 40px;
    }

    .no-image {
      background-image: $acquiesce-image;
      // background-image: url('data:image/jpeg;base64,');
      background-repeat: no-repeat;
      background-position: center;
      background-size: cover;
    }
  }
</style>
